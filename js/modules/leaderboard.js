...
const LeaderboardModule = {
    ...
    // Get aggregated scores from subcollections (skipping unavailable members)
    async getAggregatedScores() {
        const { db } = window.appUtils;
        ...
        // Process results - SKIP UNAVAILABLE MEMBERS ENTIRELY
        results.filter(Boolean).forEach(({ date, teamCode, snapshot }) => {
            snapshot.docs.forEach(doc => {
                const memberId = doc.id;
                const data = doc.data();

                // Skip unavailable members completely
                if (data.unavailable === true) {
                    return;
                }
    ...
    async loadTopTeams() {
        try {
            ...
            // Get total available member counts per team (excluding unavailable)
            const teamAvailableCounts = {};
            Object.values(scoresData).forEach(member => {
                const details = memberDetails[member.memberId];
                if (details) {
                    const teamCode = details.teamCode;
                    teamAvailableCounts[teamCode] = (teamAvailableCounts[teamCode] || 0) + 1;
                }
            });
            ...
            Object.entries(teamScores).forEach(([teamId, teamScore]) => {
                ...
                const availableMembersCount = teamAvailableCounts[teamId] || 0;

                if (teamData && availableMembersCount > 0) {
                    const activeMembersWithScores = teamScore.members.filter(m => m.hasActivity).length;

                    if (activeMembersWithScores === availableMembersCount) {
                        teams.push({
                            name: teamData.name || teamId,
                            score: teamScore.totalScore,
                            membersCount: availableMembersCount
                        });
                    }
                }
            });
    ...
    async loadTopTeamLeaders() {
        try {
            ...
            Object.entries(teamScores).forEach(([teamId, teamScore]) => {
                ...
                if (teamData?.leader && availableMembersCount > 0) {
                    const activeMembersWithScores = teamScore.members.filter(m => m.hasActivity).length;
    ...
    // Load Teams With Zero Scores (only available members with zero scores)
    async loadTeamsWithZeroScores() {
        try {
            ...
            // Step 1: Collect scores for available members only
            for (const [memberId, member] of Object.entries(scoresData)) {
                ...
                if (memberScore === 0) {
                    teamScores[teamCode].zeroScoreMembers.push({
                        name: details.name
                    });
                } else {
                    teamScores[teamCode].activeMembers.push({
                        name: details.name,
                        score: memberScore
                    });
                }
            ...
            // Step 2: Extract teams with zero-score available members
            const teamsWithZeroScores = Object.entries(teamScores)
                .filter(([_, team]) => team.zeroScoreMembers.length > 0)
                .map(([teamId, team]) => ({
                    name: teamsData[teamId]?.name || teamId,
                    leader: teamsData[teamId]?.leader || '—',
                    score: team.totalScore,
                    zeroScoreCount: team.zeroScoreMembers.length,
                    zeroScoreMembers: team.zeroScoreMembers
                }))
